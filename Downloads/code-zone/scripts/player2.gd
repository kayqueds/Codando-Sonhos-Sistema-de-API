...
@onready var hitbox_area = $HitBoxAtaque
...
func _physics_process(delta: float) -> void:
	...
	if collision:
		...
		if collider and collider.name == "Lava":
			die()

	if Input.is_action_just_pressed("invert_p2"):
		toggle_invert()
...
func die():
	...
	if is_alive:
		...
		# ESSA LINHA É OBRIGATÓRIA PARA A CENA SABER DA MORTE
